...
After editing Python files (like `jupyter_ai_braket/routes.py`):

```bash
python -m py_compile jupyter_ai_braket/__init__.py  # Check single file for syntax errors
```
...
**Python** (in `jupyter_ai_braket/*.py` files):
...
- Backend processing in `jupyter_ai_braket/routes.py`
...
- **✅ Do**: `jupyter_ai_braket/` with underscores, all lowercase
...
```typescript
const PLUGIN_ID = 'jupyter_ai_braket:plugin';
```
...
export namespace CommandIDs {
  export const openPanel = 'jupyter_ai_braket:open-panel';
  export const refreshData = 'jupyter_ai_braket:refresh-data';
}
...
- Backend routes: `jupyter_ai_braket/routes.py` (class `DataRouteHandler`)
- Frontend logic: `src/` directory
- Python package: `jupyter_ai_braket/` directory
...
1. **Read the backend first** — Check `jupyter_ai_braket/routes.py` to understand the existing API contract
...
Create RESTful endpoints in `jupyter_ai_braket/routes.py`:
...
def setup_route_handlers(web_app):
    ...
    data_route = url_path_join(base_url, "jupyter_ai_braket", "data")
...
- Define matching endpoint path strings (e.g., `"hello"`, `"get-data"`) in both `jupyter_ai_braket/routes.py` and `src/api.ts` to ensure routes sync between backend and frontend
...
2. **Write the code** (TypeScript in `src/`, styles in `style/`, Python in `jupyter_ai_braket/`)
...
5. **Install the extension** (REQUIRED for JupyterLab to recognize it):
   ...
   jupyter server extension enable jupyter_ai_braket
   ```
...
- **Output**: Creates files in `lib/` and `jupyter_ai_braket/labextension/`
...
**After editing Python** (files in `jupyter_ai_braket/`):
...
- Ask user to search for the extension ID (`jupyter_ai_braket`) to see if it loaded
...
ls -la jupyter_ai_braket/labextension/  # Should contain bundled extension
```
...
- Backend routes: `jupyter_ai_braket/routes.py`
- Server extension config: `jupyter-config/server-config/jupyter_ai_braket.json`
...
**❌ Don't**: Write frontend interfaces without first checking the backend API contract in `jupyter_ai_braket/routes.py`
...
**❌ Don't**: Use absolute imports like `from jupyter_ai_braket.routes import ...`
...
- **Plugin ID** (in `src/index.ts`): `'jupyter_ai_braket:plugin'`
- **Command IDs** (in `src/commands.ts` or `src/index.ts`): `'jupyter_ai_braket:command-name'`
...
- **CSS classes** (in `style/index.css`): `.jp-jupyter-ai-braket-ClassName`
- **API routes** (in `jupyter_ai_braket/routes.py`): `url_path_join(base_url, "jupyter_ai_braket", "endpoint")`
